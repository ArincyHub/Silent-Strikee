...
local c = {
    Background = Color3.fromRGB(22, 22, 26), -- main panel (darker)
    Secondary = Color3.fromRGB(30, 30, 35), -- cards / sections
    ...
    Accent = Color3.fromRGB(150, 130, 255), -- Premium Purple

    Toggle = {
        Enabled = Color3.fromRGB(150, 130, 255),
    ...
    Notification = {
        ...
        Timer = Color3.fromRGB(127, 255, 212)
    }
}
...
local s = {
    v0rtexd = {Width = isMobile and 500 or 600, Height = isMobile and 300 or 400},
...
local function GetConfigFolder()
    return "AcrylicConfigs/" .. game.GameId .. "/"
end
...
local function EnsureConfigFolder()
    if isfolder then
        ...
        local gameFolder = "AcrylicConfigs/" .. game.GameId
...
local function GetAutoLoadPath()
    return "AcrylicConfigs/" .. game.GameId .. "/_autoload.json"
end
...
local function GetAvailableConfigs()
    ...
    if isfolder and listfiles then
        EnsureConfigFolder()
        local folder = "AcrylicConfigs/" .. game.GameId
...
local function GetSettingsPath()
    return "AcrylicConfigs/" .. game.GameId .. "/_settings.json"
end

local function SaveLibrarySetting(key, value)
    ...
    local data = (ok and type(existing) == "table") and existing or {}
    data[key] = value
    pcall(function() writefile(path, hs:JSONEncode(data)) end)
end

local function LoadLibrarySetting(key, default)
    if not (readfile and isfile) then return default end
    local path = GetSettingsPath()
...
local function CreateNotificationContainer(screenGui)
    ...
    NotificationContainer = CreateInstance("Frame", {
        ...
        BackgroundTransparency = 1,
        Position = UDim2.new(1, -390, 0, 20),
        Size = UDim2.new(0, 370, 1, -40),
        Parent = screenGui
    })
...
function Library:Notify(config)
    ...
    local icon = config.Icon or "rbxassetid://116422175458617"
    local minWidth = 220
    local maxWidth = 360
    local measureFont = Enum.Font.GothamMedium
    local titleSize = textService:GetTextSize(title, textsize.Normal, measureFont, Vector2.new(maxWidth - 60, 1000))
    local descSize = textService:GetTextSize(description, textsize.Normal, measureFont, Vector2.new(maxWidth - 60, 1000))
    local desiredWidth = math.clamp(math.max(titleSize.X, descSize.X) + 60, minWidth, maxWidth)
    local wrappedDescHeight = math.max(19, descSize.Y)
    local desiredHeight = math.max(70, 16 + 19 + 4 + wrappedDescHeight + 12)

    local notification = CreateInstance("Frame", {
        Name = "Notification",
        BackgroundColor3 = c.Notification.Background,
        Position = UDim2.new(1, 20, 0, 0),
        Size = UDim2.new(0, desiredWidth, 0, desiredHeight),
    ...
    local descriptionLabel = CreateInstance("TextLabel", {
        ...
        TextSize = textsize.Normal,
        Size = UDim2.new(1, -60, 0, wrappedDescHeight),
        Parent = notification
    })
    ...
    local timerBar = CreateInstance("Frame", {
        ...
        Size = UDim2.new(0, desiredWidth, 0, 3),
        Parent = notification
    })
...
function Library:Toggle()
    ...
    if self._visible then
        ...
        if self.uiScale then self.uiScale.Scale = 0.85 end
        ...
        if self.uiScale then
            CreateTween(self.uiScale, {Scale = 1}, 0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
        end
        CreateTween(self.container, {GroupTransparency = 0}, 0.3)
        task.wait(0.4)
    else
        if self.uiScale then
            CreateTween(self.uiScale, {Scale = 0.85}, 0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.In)
        end
        CreateTween(self.container, {GroupTransparency = 1}, 0.25)
    ...
    if self._statusBarSetState then
        self._statusBarSetState(self._visible)
    end
    
    self._toggling = false
end

function Library:CreateStatusBar(config)
    local title = config.Title or self.title
    local logo = config.Logo or "rbxassetid://116422175458617"
    ...
    CreateCorner(statusBar, 25)
    CreateStroke(statusBar, Color3.fromRGB(45, 45, 50), 0.3)
    
    -- Logo
    local logoImg = CreateInstance("ImageLabel", {
        Name = "Logo",
        Image = logo,
        BackgroundTransparency = 1,
    ...
    local titleLabel = CreateInstance("TextLabel", {
        ...
        Text = title,
        FontFace = f.Regular,
        TextColor3 = Color3.fromRGB(127, 255, 212),
        TextXAlignment = Enum.TextXAlignment.Left,
        RichText = true,
        BackgroundTransparency = 1,
    ...
    -- Stats Label (Time | FPS)
    local statsLabel = CreateInstance("TextLabel", {
        Name = "Stats",
        Text = "00:00:00 | 0 FPS",
        FontFace = f.Regular,
    ...
    task.spawn(function()
        while statusBar.Parent do
            ...
            statsLabel.Text = timeStr .. " | " .. fps .. " FPS"
            task.wait(0.5)
        end
    end)
    
    -- Action Button (Pill)
    local pillBtn = CreateInstance("Frame", {
        Name = "ActionButton",
    ...
    CreateCorner(pillBtn, 15)
    CreateStroke(pillBtn, Color3.fromRGB(150, 130, 255), 0.4)
    ...
    clickArea.MouseButton1Click:Connect(function()
        toggled = not toggled
        btnLabel.Text = toggled and "Open" or "Close"
        
        CreateTween(pillBtn, {BackgroundColor3 = toggled and Color3.fromRGB(45, 45, 55) or Color3.fromRGB(30, 30, 35)}, 0.2)
...
function Library:_CreateMainv0rtexd()
    ...
    self.container = CreateInstance("CanvasGroup", {
        Name = "Container",
    ...
    self.uiScale = CreateInstance("UIScale", {
...
function Library:SaveConfig(configName)
    ...
    local success, err = pcall(function()
        writefile("AcrylicConfigs/" .. game.GameId .. "/" .. configName .. ".json", hs:JSONEncode(configData))
    end)
...
function Library:LoadConfig(configName)
    ...
    local path = "AcrylicConfigs/" .. game.GameId .. "/" .. configName .. ".json"
...
function Library:SetAutoSave(enabled)
    ...
    if enabled then
        if self._autoSaveRunning then return end  -- prevent duplicate loops
...
function Library:GetAutoLoadConfig()
    ...
    local configName = decoded.config
...
function Library:SetAutoLoadConfig(configName)
    if not writefile then
        self:Notify({
            Title = "Error",
            Description = "Autoload is not supported",
            Duration = 3
        })
        return false
    end
    ...
    local ok = pcall(function()
        writefile(GetAutoLoadPath(), hs:JSONEncode({config = tostring(configName)}))
    end)

    if ok then
        self:Notify({
            Title = "Autoload Set",
            Description = "Autoload config: " .. tostring(configName),
            Duration = 2
        })
        return true
    end
    ...
    self:Notify({
        Title = "Error",
        Description = "Failed to set autoload config",
        Duration = 3
    })
    return false
end
...
function Library._CreateTab(section, name, icon)
    ...
    local indicator = CreateInstance("ImageLabel", {
        Name = "Indicator",
        BackgroundTransparency = 1,
        Image = "rbxassetid://105995086404082",
        ImageColor3 = c.TextDark,
        AnchorPoint = Vector2.new(0, 0.5),
    ...
    clickBtn.MouseButton1Click:Connect(function()
        Library._SelectTab(section._library, tab, tabBtn, tabStroke, iconLabel, tabText, textGradient, indicator)
    end)
    ...
    function tabMethods:CreateSplit()
        return Library._CreateSplit(self)
    end
...
function Library._SelectTab(lib, tab, btn, stroke, icon, textLabel, textGradient, indicator)
    ...
    if lib.currentTab then
        ...
        if lib.currentTab.indicator then
            CreateTween(lib.currentTab.indicator, {ImageColor3 = c.TextDark}, animationspeed.Fast)
        end
    ...
    if indicator then
        CreateTween(indicator, {ImageColor3 = c.Text}, animationspeed.Fast)
    end
...
function Library._CreateSplit(tab)
    local splitFrame = CreateInstance("Frame", {
        Name = "SplitFrame",
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y,
    ...
    CreateListLayout(splitFrame, 10, Enum.SortOrder.LayoutOrder, Enum.FillDirection.Horizontal)

    local function createColumn(side)
        local col = CreateInstance("Frame", {
            Name = side .. "Column",
            BackgroundTransparency = 1,
        ...
        CreateListLayout(col, 10, Enum.SortOrder.LayoutOrder)
        
        local colMethods = {}
        local colTab = {content = col, _library = tab._library}

        function colMethods:CreateSection(config)
            ...
            CreatePadding(sectionFrame, 10, 10, 10, 10)
            CreateListLayout(sectionFrame, 8, Enum.SortOrder.LayoutOrder)
            ...
            CreateInstance("TextLabel", {
                ...
                Size = UDim2.new(1, -iconOffset - 25, 1, 0),
                TextSize = textsize.Normal,
                Parent = header
            })
            ...
            CreateListLayout(container, 8, Enum.SortOrder.LayoutOrder)

            headerBtn.MouseButton1Click:Connect(function()
                expanded = not expanded
                container.Visible = expanded
                CreateTween(arrow, {Rotation = expanded and 0 or 180}, animationspeed.Normal)
            end)

            local sectionTab = {content = container, _library = tab._library}
            local sectionMethods = {}
            
            function sectionMethods:CreateToggle(cfg) return Library._CreateToggle(sectionTab, cfg) end
            function sectionMethods:CreateSlider(cfg) return Library._CreateSlider(sectionTab, cfg) end
            function sectionMethods:CreateButton(cfg) return Library._CreateButton(sectionTab, cfg) end
            function sectionMethods:CreateDropdown(cfg) return Library._CreateDropdown(sectionTab, cfg) end
            function sectionMethods:CreateKeybind(cfg) return Library._CreateKeybind(sectionTab, cfg, tab._library) end
            function sectionMethods:CreateColorPicker(cfg) return Library._CreateColorPicker(sectionTab, cfg) end
            function sectionMethods:CreateTextBox(cfg) return Library._CreateTextBox(sectionTab, cfg) end
            function sectionMethods:CreateParagraph(cfg) return Library._CreateParagraph(sectionTab, cfg) end
            
            return sectionMethods
        end

        -- Standard elements can also be added directly to column
        function colMethods:CreateToggle(cfg) return Library._CreateToggle(colTab, cfg) end
        function colMethods:CreateSlider(cfg) return Library._CreateSlider(colTab, cfg) end
        function colMethods:CreateButton(cfg) return Library._CreateButton(colTab, cfg) end
        function colMethods:CreateDropdown(cfg) return Library._CreateDropdown(colTab, cfg) end
        function colMethods:CreateParagraph(cfg) return Library._CreateParagraph(colTab, cfg) end
...
function Library._CreateSlider(tab, config)
    ...
    local valueInput = CreateInstance("TextBox", {
        Name = "ValueInput",
        FontFace = f.Regular,
        TextColor3 = c.Text,
    ...
    local function quantize(value)
        ...
        local clamped = math.clamp(snapped, min, max)
        if precision > 0 then
            local mul = 10 ^ precision
            clamped = math.floor(clamped * mul + 0.5) / mul
        else
    ...
    local function UpdateSlider(input)
        ...
        applyValue(min + (max - min) * relativeX, true)
    end
...
function Library._CreateDropdown(tab, config)
    ...
    local toolbar = CreateInstance("Frame", {
        ...
        Size = UDim2.new(1, 0, 0, multiSelect and 24 or 0),
        Visible = multiSelect,
        ZIndex = 9002,
        Parent = overlay
    })
    if multiSelect then
        CreateInstance("UICorner", {CornerRadius = UDim.new(0, 4), Parent = toolbar})
    end

    local function mkToolBtn(txt, xPos)
    ...
    local selectAllBtn = multiSelect and mkToolBtn("Select All", 8) or nil
    local clearBtn = multiSelect and mkToolBtn("Clear", 84) or nil
    ...
    local optionsScroll = CreateInstance("ScrollingFrame", {
        ...
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        Position = UDim2.new(0, 0, 0, SEARCH_HEIGHT + (multiSelect and 24 or 0)),
        Size = UDim2.new(1, 0, 1, -(SEARCH_HEIGHT + (multiSelect and 24 or 0))),
        CanvasSize = UDim2.new(0, 0, 0, 0),
        AutomaticCanvasSize = Enum.AutomaticSize.Y,
        ScrollBarThickness = 3,
    ...
    local function RefreshOverlaySize(count)
        local extra = multiSelect and 24 or 0
        local h = SEARCH_HEIGHT + extra + math.min(count, MAX_VISIBLE) * OPTION_H
    ...
    if multiSelect and selectAllBtn and clearBtn then
...
function Library._CreateInfoSection(tab, config)
    ...
    local hubLogo        = config.HubLogo        or "rbxassetid://116422175458617"
...
function Library._CreateConfigSection(tab)
    ...
    local function refreshList()
        local all = lib:GetConfigs()
        if #all == 0 then
            all = {"--"}
        end
        configDropdown:Refresh(all)
    end

    local function normalizeName()
        local name = (configNameBox:GetText() or ""):gsub("^%s+", ""):gsub("%s+$", "")
        return name
    end

    local autoloadLabel = Library._CreateParagraph(tab, {
        Title = "Autoload",
        Content = "Current autoload config: none"
    })

    local function refreshAutoloadLabel()
        local cfg = lib:GetAutoLoadConfig()
        autoloadLabel:SetContent("Current autoload config: " .. (cfg or "none"))
    end

    Library._CreateButton(tab, {
        Name = "Create Config",
        Callback = function()
            local configName = normalizeName()
            if configName == "" then
                lib:Notify({
                    Title = "Invalid Name",
            ...
            if isfile and isfile(path) then
                lib:Notify({
                    ...
                    Description = "Use Saved Config for existing configs",
                    Duration = 3
                })
                return
            end

            if lib:SaveConfig(configName) then
                refreshList()
            end
        end
    })
        ...
        Callback = function()
            ...
            if isfile and not isfile(path) then
                lib:Notify({
                    ...
                    Description = "Create or Saved config first",
                    Duration = 3
                })
                return
            end

            if lib:SetAutoLoadConfig(configName) then
    ...
    Library._CreateToggle(tab, {
        Name = "Auto Save",
        Default = lib._autoSave or false,
        Callback = function(enabled)
            lib:SetAutoSave(enabled)
            lib:Notify({
                Title = "Auto Save",
                Description = "Auto save " .. (enabled and "enabled" or "disabled"),
                Duration = 2,
                Icon = "rbxassetid://10723356507"
            })
        end
    ...
    task.defer(function()
        local cfg = lib:GetAutoLoadConfig()
        if cfg and cfg ~= "" then
            configNameBox:SetText(cfg)
            lib._currentConfig = cfg
            lib:LoadConfig(cfg)
        end
    end)
...
local window = Library.new("<font color='rgb(127, 255, 212)'>Zen Hub</font> - <font color='rgb(150, 130, 255)'>Premium</font>", "ZenHubConfigs")
window:SetToggleKey(Enum.KeyCode.RightControl)

local MainSection = window:CreateSection("Main")
local MainTab = MainSection:CreateTab("Home", "rbxassetid://10734898355")


-- 1. Create the split layout on the Tab
local Left, Right = MainTab:CreateSplit()
local MainFarm = Left:CreateSection({
    Name = "Main Farm",
...
-- 3. Create a box-style section in the Right column
local MobFarm = Right:CreateSection({
    Name = "Mob Farm"
})


local InfoTab1 = window:CreateSection("General")
local InfoTab = InfoTab1:CreateTab("Information", "rbxassetid://116422175458617")
...
window:Notify({
    ...
    Duration = 3,
    Icon = "rbxassetid://116422175458617"
})
...
local StatusBar = window:CreateStatusBar({
    Title = "<font color='rgb(127, 255, 212)'>Zen Hub</font> - <font color='rgb(150, 130, 255)'>Premium</font>",
