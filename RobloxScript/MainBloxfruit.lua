...
local Alc = loadstring(game:HttpGet("https://raw.githubusercontent.com/Zenhubtop/zen_hub_pr/refs/heads/main/364633664396263"))()
local today = os.date("%Y-%m-%d")
local Window = Alc:NewWindow(
    "Version : 2.1 / Date: " .. today, 
...
local tab2 = MenuFunctions:AddTab("Discord", 10723415903)
local Mainfarm = MenuFunctions:AddTab("Farm", 10723407389)
local Stats = MenuFunctions:AddTab("Stats", 10734950020)
local Fishing = MenuFunctions:AddTab("Fishing", 127664059821666)
local QuestItem = MenuFunctions:AddTab("Quest/Items", 15557776256)
local Racev4 = MenuFunctions:AddTab("Race V4", 14477517268)
local Raids = MenuFunctions:AddTab("Raid/Fruits", 10734932295)
local Eventtab = MenuFunctions:AddTab("Sea Event", 10734941354)
local Teleport = MenuFunctions:AddTab("Teleport", 10734886004)
local Status = MenuFunctions:AddTab("Status", 10723415903)
local Visual = MenuFunctions:AddTab("Visual", 6034467796)
local Shop = MenuFunctions:AddTab("Shop", 10734952479)
local Misc = MenuFunctions:AddTab("Misc", 10734949856)

local Home_Left = tab2:AddSection("Info and Ui settings",true)
...
local settingfarm = Stats:AddSection('Config Farm')

settingfarm:AddDropdown({
    Name = "Select Tool",
    Description = "Choose the tool you want to use",
    Option = {"Melee", "Sword", "Gun", "Blox Fruit"},
    Default = "Melee",
    ...
    while task.wait() do
        pcall(function()
            ...
            for _,v in pairs(workspace.Enemies:GetChildren()) do
                ...
                and (v.Name == MonFarm or v.Name == Mon)
                and (v.HumanoidRootPart.Position - PosMon.Position).Magnitude <= 320 then
                    table.insert(mobs,v)
                end
            end
            ...
            for i,mob in ipairs(mobs) do
                ...
                if i == 1 then
                    ...
                    local a = angle + (i * (math.pi*2/count))
                    ...
                    root.CFrame = PosMon * CFrame.new(offset)
                end
            end
        end)
    end
...
local Statspoints = Stats:AddSection('Player Stats')
...
Statspoints:Textbox("Point", "Enter points (0 - 100)", function(value)
	local number = tonumber(value)
	if number and number >= 0 and number <= 100 then
		PointStats = number
		print("Points to allocate:", number)
	else
		warn("Invalid point value. Please enter a number between 0 and 100.")
	end
end)
...
local v485 = Mainfarm:AddSection('Main farm')
...
if World3 then
local Tyris = Mainfarm:AddSection('Tyrant of the Skies')
local v548 = Tyris:AddLabel("Check Eyes Status")
task.spawn(function()
...
local Bones = Mainfarm:AddSection('Farm Bones')
local v589 = Bones:AddLabel("Check Bone")
task.spawn(function()
    while task.wait(1) do
        pcall(function()
...
local Katakurii = Mainfarm:AddSection('Katakuri Boss')
local v606 = Katakurii:AddLabel("Check Cake Prince")
task.spawn(function()
    while task.wait(1) do
        pcall(function()
...
local appberr = Mainfarm:AddSection('Auto Farm Chest And Berry')
...
local Bosess = Mainfarm:AddSection('Boss Farm')
local v657 = Bosess:AddLabel("Boss Spawn Status")
task.spawn(function()
    while task.wait(1) do
        pcall(function()
...
local mat = Mainfarm:AddSection('Material')
local v664 = {}
if not World1 then
    if World2 then
...
local Fish = Fishing:AddSection('Auto Fishing')
Fish:AddToggle({
    Name ="Auto Fishing",
    Description = "",
    Default = false,
...
Fish:AddDropdown({
    ...
    Description = "",
    Option = {"Basic Bait", "Kelp Bait", "Good Bait", "Abyssal Bait", "Frozen Bait", "Epic Bait", "Carnivore Bait"},
    Default = "Basic Bait",
...
Fish:AddDropdown({
    ...
    Description = "",
    Option = {"Fishing Rod", "Gold Rod", "Shark Rod", "Shell Rod", "Treasure Rod"},
    Default = "Fishing Rod",
...
if World1 then
    local Itemsq = QuestItem:AddSection('Quest Sea 1')
    Itemsq:AddToggle({
        Name = "AutoSecondSea",
        Description = "",
        Default = false,
    ...
    local grey = QuestItem:AddSection('Boss Greybeard')
    grey:AddToggle({
        Name = "Kill Greybeard",
        Description = "",
        Default = false,
    ...
    local Swords = QuestItem:AddSection('Quest Sword')
    Swords:AddToggle({
        Name = "Auto Get Saber",
        Description = "",
        Default = false,
...
if World2 then
    local Qsea2 = QuestItem:AddSection('Quest Sea 2')
    Qsea2:AddToggle({
...
local beard = QuestItem:AddSection('Boss Dark Beard')
    beard:AddToggle({
    ...
    local hakis = QuestItem:AddSection('Auto Buy Haki')
    hakis:AddToggle({
    ...
    local qswords = QuestItem:AddSection('Quest Sword')
    qswords:AddToggle({
...
if World3 then
    local Seas3 = QuestItem:AddSection('Farm Sea 3')
    Seas3:AddToggle({
    ...
    do
        ...
        local Guitar = QuestItem:AddSection('Quest Skull Guitar')
        Guitar:AddToggle({
        ...
        local Cdks = QuestItem:AddSection('Dual Cursed Katana')
        Cdks:AddToggle({
            Name = "Auto Cdk [Beta]",
            Description = "",
            Default = false,
        ...
        local qsword3 = QuestItem:AddSection('Quest Sword')
        qsword3:AddToggle({
    ...
    local race = Racev4:AddSection('Teleports')
...
local trials = Racev4:AddSection('Trial V4')
trials:AddButton({
    Name ="Auto Race Door",
    Value = false,
    Callback = function()
...
local Skillt = Racev4:AddSection('Skills For Trial')
Skillt:AddToggle({
    Name = "Auto Skill Z",
    Description = "",
    Default = false,
...
local Seavent = Eventtab:AddSection('Sea Events')
Seavent:AddToggle({
...
local dojot = Eventtab:AddSection('Volcanic Island')
dojot:AddButton({
...
local v850 = dojot:AddLabel("Check Prehistoric Island")
task.spawn(function()
    while task.wait(1) do
        pcall(function()
...
local skillvent = Eventtab:AddSection('Auto Skill Sea Event')
...
local golemss = Eventtab:AddSection('Auto Kill Golem')
golemss:AddToggle({
...
local EGGBONE = Eventtab:AddSection('Auto Collect Bone,Egg')
...
local Kitsu = Eventtab:AddSection('Kitsune Island')
local v939 = Kitsu:AddLabel("Check Kitsune Island")
task.spawn(function()
    while task.wait(1) do
        pcall(function()
...
local Mirge = Eventtab:AddSection('Mirage Island')

local v984 = Mirge:AddLabel("Check Mirage Island")
task.spawn(function()
    while task.wait(1) do
        pcall(function()
...
local Fruitss = Raids:AddSection("Fruits")
...
local cfruit = Raids:AddSection('Check Stock Fruits')
...
local v1112 = cfruit:AddLabel("Stock")
task.spawn(function()
...
if World1 then
    cfruit:AddLabel("Raids only works in Sea 2 and 3")
else
local Dungeonf = Raids:AddSection('Dungeon Raid')
...
local Lawr = Raids:AddSection('Law Raid')
Lawr:AddButton({
...
local Tpland = Teleport:AddSection('Teleport Island')
...
local Tpsea = Teleport:AddSection('Teleport Sea')
Tpsea:AddButton({
    Name = "Sea 1",
    Description = "",
    Callback = function()
...
local Sttus = Status:AddSection('Check Status')

local TyrantStatus = Sttus:AddLabel("Tyrant of the Skies")
spawn(function()
    pcall(function()
        while wait() do
...
local CheckRip = Sttus:AddLabel("Rip_Indra")
spawn(function()
    while wait() do
        pcall(function()
...
local CheckDoughKing = Sttus:AddLabel("Dough King")
spawn(function()
    while wait() do
        pcall(function()
...
local EliteHunter = Sttus:AddLabel("Elite Hunter")
spawn(function()
    while wait() do
        pcall(function()
...
local Pullever = Sttus:AddLabel("Pull Lever")
spawn(function()
    while wait() do
        pcall(function()
...
local FM = Sttus:AddLabel("Full Moon")
spawn(function()
    while wait() do
        pcall(function()
...
local LegendarySword = Sttus:AddLabel("Legendary Sword")
spawn(function()
    pcall(function()
        while wait() do
...
local Bone = Sttus:AddLabel("Bone")
spawn(function()
    pcall(function()
        while wait() do
...
local AIMBT = Visual:AddSection('Aimbot Nearest')
...
local AIMBT2 = Visual:AddSection("Aimbot skill V2")
local v1 = loadstring(game:HttpGet("https://raw.githubusercontent.com/PlockScripts/Aimbot-skill-config/refs/heads/main/Aimbot.lua"))()
...
local eSPS = Visual:AddSection("Esp")
...
eSPS:AddToggle({
   ...
   local visuals = Visual:AddSection("Visual")
...
local fisghstyle = Shop:AddSection("Fighting Style")
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
local Eventcraft = Shop:AddSection("Buy Sea Event Crafting")
Eventcraft:AddButton({
...
local bodygyro = Shop:AddSection("Buy Haki,Soru...")
bodygyro:AddButton({
...
local Swg = Shop:AddSection("Buy Sword,Gun")
Swg:AddButton({
...
local tatrace = Shop:AddSection("Reset Stats , Random Race")
tatrace:AddButton({
    Name ="buy Ghoul",
    Description = "",
    Callback = function()
...
local server = Misc:AddSection("Join Server")
server:Textbox("Job ID","Paste the Job ID here..." ,function(p215)
 if p215 ~= "" then
...
local serverhop = Misc:AddSection("Server")
serverhop:AddButton({
    Name ="Rejoin Server",
    Callback = function()
...
serverhop:AddButton({Name ="Server Hop", Callback = function()
    Hop()
end})
...
local teams = Misc:AddSection("Team")
teams:AddButton({
...
local menus = Misc:AddSection("Menu")
menus:AddButton({
...
local playerstg = Misc:AddSection("Local-Player")
...
local othermisc = Misc:AddSection("Others")
...
PosY = 30
othermisc:AddToggle({Name ="Dodge No CD", Value = false, Callback = function(v1178)
...
othermisc:AddToggle({Name ="Infinite Geppo", Value = false, Callback = function(v1183)
    InfiniteGeppo = v1183
end})
spawn(function()
    while task.wait(1) do
...
othermisc:AddButton({
    ...
    Description = "",
    Callback = function()
        for _, v1220 in ipairs(codesss) do
            local v1221 = {v1220}
            do
